...
class Notification(BaseModel):
    ...
    channel: Mapped[NotificationChannel] = mapped_column(default=NotificationChannel.PUSH)
    ...
    target_committee_id: Mapped[UUID | None] = mapped_column(ForeignKey("committees.id"), nullable=True)

    # Store provider response (e.g. FCM message ID or SendGrid ID)
    provider_reference: Mapped[str | None] = mapped_column(String(255), nullable=True)

    # Who triggered the broadcast
    sender_id: Mapped[UUID | None] = mapped_column(ForeignKey("members.id"), nullable=True)
