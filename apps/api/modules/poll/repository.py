...
class PollRepository:
    ...
    def _base_query(self) -> Select:
        return select(Poll).options(selectinload(Poll.options)).where(
    ...
    async def create(self, data: dict[str, Any], options_data: list[dict[str, str]], created_by: UUID | None = None) -> Poll:
        poll = Poll(
    ...
    async def get_all_paginated(self, offset: int = 0, limit: int = 20) -> tuple[list[Poll], int]:
        ...
        count_stmt = select(func.count()).select_from(Poll).where(
            Poll.tenant_id == self.tenant_id,
            Poll.is_deleted == False,  # noqa: E712
        )
    ...
    async def has_voted(self, poll_id: UUID, member_id: UUID) -> bool:
        stmt = select(PollVote).where(
            ...
            PollVote.tenant_id == self.tenant_id
        )
        result = await self._session.execute(stmt)
